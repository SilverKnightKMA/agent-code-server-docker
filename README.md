...
By default, `omp` and other managed tools are only installed into the volume when you set `AGENT_CODE_SERVER_AUTOINSTALL: "true"` in compose or run `npm run --prefix /opt/agent-code-server/managed-tools managed-tools:init` inside the container. That managed init now also installs the mounted Paseo skills pack and the Pi extension pack (which carries its external npm packages, e.g. `pi-mcp-adapter`, since pi-config v1.4.20) for the managed agent CLIs.
...
| **2. Managed mounted** | omp, pi, opencode, claude, codex, droid, copilot, TypeScript LSP, Go, Rust, gh, yq, ripgrep, Paseo skills, pi ext pack (carries pi-mcp-adapter/pi-web-access) | Volume data/ |
