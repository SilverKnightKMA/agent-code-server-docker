...
CODE_SERVER_OMP_CONFIG_CACHE_DIR="${XDG_STATE_HOME}/code-server-omp/config"
CODE_SERVER_OMP_TMPDIR="${XDG_STATE_HOME}/code-server-omp/tmp"
...
export CODE_SERVER_OMP_CONFIG_SOURCE="baked"
...
mkdir -p \
  ...
  "${PYTHONUSERBASE}/bin" \
  "${XDG_DATA_HOME}" \
  "${XDG_STATE_HOME}/code-server-omp" \
  "${MANAGED_NPM_PREFIX}" \
  "${CONFIG_BASE}/.ssh" \
